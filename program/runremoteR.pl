...
my $sth1 = $dbh->prepare("SELECT JOBID, SampleInput, DoseInput, NetworkInput, Method, Name, Email FROM Jobs where status = 0 order by CreateTime desc limit 1")
...
my $stm0 = $dbh->prepare("update drugCombinationRemoteR.Jobs set Status = 1 where JOBID = '" . $jobid . "';");
$stm0->execute();
$stm0->finish();
$dbh->disconnect();
...
# --------------------------------  run program  --------------------------------------
system("Rscript process_pipline.R \"" . $jobid . "\" -p " . $method);
