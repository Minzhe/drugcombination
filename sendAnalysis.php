...
if (True) {
    ...
    if (True) {     //match the verification code

        if ($_FILES["userfile"]["size"] > 0 && $_FILES["dosefile"]["size"] > 0) {     //check if uploaded file
            ...
            if ($isFileSizeNotGood == 1 || $isFileTypeNotGood == 1 || $isDoseFileTypeNotGood == 1) {
                echo "<script>location.href='onlineAnalysis.php?isFileTypeNotGood=$isFileTypeNotGood&isFileSizeNotGood=$isFileSizeNotGood&isDoseFileTypeNotGood=$isDoseFileTypeNotGood'</script>";
                exit();
            }
            ...
            echo "<script>location.href='./onlineAnalysis.php?isNoDataFile=$isNoDataFile&isNoDoseFile=$isNoDoseFile'</script>";
            exit();
        }

        if ($_FILES["geneNetfile"]["size"] > 0) {     //check if uploaded network file
        ...
        // set job status as 0. 0 means new job, 1 means job taken, 2 means job done, 3 means job failed.
        ...
        echo "location.href=\"./onlineAnalysis.php?isWrongVerification=1\"";
        echo "</script>";
    }
    exit();
}
